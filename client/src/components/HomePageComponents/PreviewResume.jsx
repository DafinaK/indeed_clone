...
export default function PreviewResume({ resume }) {
  ...
  return (
    <>
      {resume ? (
        <div className='relative w-full mx-auto bg-blue-200 p-6 rounded-lg shadow-md flex flex-col gap-4'>
          <h2 className='text-xl font-bold text-blue-900'>Preview Resume</h2>
          <p className='text-blue-700'>View your resume to ensure you wrote the correct information.</p>
          {resume.mime == 'application/pdf' ? (
            <>
              <button onClick={handleOpenPreview} className='py-2 mt-2 text-white bg-blue-600 rounded hover:bg-blue-700'>
                Preview Resume
              </button>
              <div
            ...
            <a href={`http://localhost:5000/resumes/${resume.s3Key}`} className='text-center py-2 mt-2 text-white bg-blue-600 rounded hover:bg-blue-700'>
              Download Resume
            </a>
          )}
        </div>
