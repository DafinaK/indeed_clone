...
export default function DashboardNavSection() {
  ...
  const checkActive = (path, matchExact = false) => {
    if (matchExact) {
      ...
      <section className='navSection bg-white'>
        <nav className='dashboardNav max-w-[1200px] mx-auto'>
          <ul className='flex justify-between gap-10 flex-wrap'>
            <li>
              <NavLink to='/company/dashboard' className={({ isActive }) => `flex gap-2 p-5 items-center ${isActive ? 'border-b border-jobportal-cyan text-jobportal-cyan' : ''}`}>
                {/* Squares 2x2 SVG */}
