...
import InterviewJobItem from './InterviewJobItem';
import JobItem from '../Dashboard/JobItem';

export default function InterviewTableSection({ userData }) {
  ...
  useEffect(() => {
    const fetchJobData = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/jobposts/interviews/${userData.ID}`);
  ...
  const filteredJobPosts = jobData.filter((job) => job.positionName.toLowerCase().includes(searchQuery.toLowerCase())).filter((job) => job.interviewActive === showActiveJobs);
  ...
  const handleToggleJobs = (interviewActive) => {
    setShowActiveJobs(interviewActive);
        ...
        <div className='container mx-auto '>
          <div className={`applicantListItem flex items-center justify-between gap-5 w-full p-0 mt-10`}>
            <div className='flex items-start gap-10 w-full pl-5 pr-5'>
              ...
              <p className='text-gray-600 w-1/12 font-bold'>Interview Time</p>
              <p className='text-gray-600 w-2/12 font-bold'>Address</p>
              <p className='text-gray-600 w-2/12 font-bold'>Link</p>
              <p className='text-gray-600 w-1/12 font-bold'>Candidates</p>
          ...
          <div className='flex flex-col '>
            {paginatedJobPosts.map((jobData) => (
              <div className='flex flex-row pt-2 gap-5 w-full'>
                <InterviewJobItem className='w-12/12' key={jobData.ID} jobData={jobData} />
              </div>
            ))}
          </div>
