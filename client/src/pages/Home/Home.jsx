...
const Home = () => {
  ...
  const handleFormSubmit = (e) => {
    e.preventDefault();
    navigate(`/find-jobs?q=${encodeURIComponent(searchQuery)}`); //kur perdoruesi kerkon nje pune ai ridrejtohet ne fq kerkimit
  };

  const handleQueryChange = (e) => {
    setSearchQuery(e.target.value);     //azhurnon tekstin e kerimit derisa perdoruesi shkruan
  };
  ...
  return (
    <>
      ...
      <div className='bg-[#f1f2f4] mt-[100px]'>
        <div className='font-medium text-[42px] text-center mb-[100px] pt-[100px]'>How does Indeed Clone Work ?</div>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
            {jobFields.map((field) => (
              <Link
                ...
                key={field.id}
                className='flex items-center p-4 bg-blue-50 rounded-md shadow-sm hover:shadow-lg cursor-pointer transition-shadow duration-300'
              >
                <div className='flex-shrink-0 w-16 h-16 border-blue-700 border rounded-full'>
                  <img
        ...
        <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6'>
          {companies.map((company, index) => (
            ...
            >
              ...
              <div>
                <button className='w-full py-2 px-4 bg-blue-100 text-blue-700 font-medium rounded-md hover:bg-blue-200 transition-colors duration-300'>Open Position</button>
              </div>
            </Link>
          ))}
        </div>
